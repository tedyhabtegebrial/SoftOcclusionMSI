...
import numpy as np
import os
from thirdparty.nex_utils.sfm_utils import SfMData
from thirdparty.nex_utils.video_path import webGLspiralPath, deepviewInnerCircle
import torch as pt
...
class OrbiterDataset(Dataset):
  def __init__(self, dataset, ref_img, scale, dmin,
    ...
    self.cache_size = 50
    self.ref_img = self.fromCache(self.dataset + "/" +
                                  self.sfm.ref_img['path'], self.scale)
    self.ref_img = np.transpose(self.ref_img, [2, 0, 1]).astype(np.float32)
    if np.max(self.ref_img) > 1:
      self.ref_img /= 255.0
  ...
  def __getitem__(self, idx):
    ...
    k_ref[0, 0] = cam['fx']
    k_ref[1, 1] = cam['fy']
    k_ref[0, 2] = cam['px'] + self.input_offset
    k_ref[1, 2] = cam['py'] + self.input_offset
    pose = np.concatenate([im['r'], im['t']], axis=1)
    ...
    feature = {
      ...
      'cy': cam['py'],
      'path': im['path'],
      'ref_rgb': 2*(self.ref_img - 0.5),
      'k_targ': k_targ,
      'k_ref': k_ref,
      'r_s': self.sfm.ref_img['r'],
      't_s': self.sfm.ref_img['t'],
      }
